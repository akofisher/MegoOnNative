    ...
    fetch(API, {
      ...
      db.json().then(json => {
        ...
        if (json.status === 'success') {
          ...
          setLoading(false)
        } else if (json.status === 'fail' && json.comment === 'Cant Get Order History') {
          setLoading(false)
        }
      })
    })
      ...
      {loading == true ? (
        ...
        <React.Fragment>
          ...
          <ScrollView
            ...
            {USERORDERS.length > 0 ? (
              USERORDERS.map((val, idx) => {
                return (
                  <OrdersCard key={idx} cart={val} />
                )
              })
            ) : (
              <Text style={styles.CartHeader}>შეკვეთები ცარიელია</Text>
            )}

...
import { selectDownChildSeg, selectSegmentsUID } from '../store/Segments/SegSelector'
import { setDownChildSeg, setSegmentsUID } from '../store/Segments/SegActionCreat'
...
const DownChildCategScreen = ({ route, navigation }) => {
    const { DownChildId, Name } = route.params
    const nav = useNavigation()
    const segUID = useSelector(selectSegmentsUID)
    const dispatchh = useDispatch()
    const [loading, setLoading] = useState(true)
    const SEGMENT = useSelector(selectDownChildSeg)
    ...
    const toSegmentsDinamic = (UID, catName) => {
        dispatchh(setSegmentsUID(UID))
        nav.navigate('CategoryNavigator', { screen: 'ProdBySeg', params: { CategId: UID, Name: catName } })
    ...
    useEffect(() => {
        ...
        fetch(API, {
            ...
            body: JSON.stringify({
                ApiMethod: 'GetChildCategories',
                controller: 'Category',
                pars: {
                    lang: 'GE',
                    PARENT_ID: DownChildId
                },
            }),
        }).then(db => {
            db.json().then(json => {
                if (json.status === 'success') {
                    ...
                    dispatchh(setDownChildSeg(json.data))
                    setLoading(false)
...
export default DownChildCategScreen